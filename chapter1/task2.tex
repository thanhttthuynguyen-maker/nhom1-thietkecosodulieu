...
\section*{A. Nhóm Quản trị \& Kết nối}

\textbf{User (Thực thể mạnh):} Người dùng cuối trong hệ thống (Developer, Staff, Manager, Tech Lead, Admin).

\textbf{Thuộc tính Phức hợp (Composite Attribute):}

\begin{itemize}
    \item \textbf{FullName} (Phức hợp: FirstName, LastName)
\end{itemize}

\textbf{Thuộc tính Đa trị (Multi-valued Attribute)}
...
\textbf{Thuộc tính Đơn (Simple / Atomic Attribute)}

\begin{itemize}
    \item \textbf{user\_id (PK):} Định danh duy nhất cho từng người dùng.
...
\textbf{Permission (Thực thể mạnh):} Quyền hạn chi tiết trên hệ thống (VD: DOC\_APPROVE, TEMPLATE\_EDIT).
...
\begin{itemize}
    ...
    \item \textbf{role\_id (FK):} Khóa ngoại trỏ đến vai trò được gán quyền (roles).
    \item \textbf{permission\_id (FK):} Khóa ngoại trỏ đến quyền hạn cụ thể (permissions).
...
\textbf{Thuộc tính Khóa (Key Attribute):}

\begin{itemize}
    \item \textbf{connection\_id:} Định danh duy nhất cho bản ghi kết nối OAuth.
\end{itemize}

\textbf{Thuộc tính Đơn / Nguyên tố (Simple / Atomic Attributes):}
...
\section*{B. Nhóm Mã nguồn \& Cấu trúc Code}
...
\begin{itemize}
    \item \textbf{AuthorInfo:} Thông tin tác giả thực hiện commit (gồm AuthorName, AuthorEmail).
\end{itemize}

\textbf{Thuộc tính Đơn (Simple / Atomic):}
...
\begin{itemize}
    \item \textbf{position:} Vị trí của thực thể trong tập tin, gồm 2 thuộc tính con:
    \begin{itemize}
...
\section*{C. Nhóm Khuôn mẫu Tài liệu (Template Engine Domain)}
...
\section*{D. Nhóm Tài liệu \& Vòng đời Dữ liệu (Documentation Domain)}
...
\begin{itemize}
    ...
    \item \textbf{updated\_at:} Thời điểm cập nhật tài liệu.
...
\begin{itemize}
    ...
    \item \textbf{ticket\_codes:} Danh sách các mã ticket/thẻ công việc liên quan từ các hệ thống bên ngoài (như Jira Ticket ID, GitHub Issue ID, ví dụ: PROJ-123, \#45), được lưu dưới dạng chuỗi văn bản đơn giản (Nullable, phân cách bằng dấu phẩy). Thuộc tính này giúp ghi nhận trực tiếp ngữ cảnh phát triển trên từng phiên bản tài liệu mà không cần duy trì thêm một thực thể phụ thuộc riêng biệt.
...
\section*{E. Nhóm Kiểm duyệt \& Phê duyệt (Review \& Governance Domain)}
...
\section*{F. Nhóm AI Evidence, Giám sát \& Kiểm toán (AI Evidence \& Observability Domain)}
